...
3. 监听器        ----观察者模式
    1) ServletContextListener - 监听ServletContext对象的创建和销毁的过程。ServletContext对象在tomcat容器启动的时候创建，在tomcat容器关掉后，销毁
		应用：IOC的工厂应该在上下文初始化的时候就准备就绪(初始化)，当别人需要组件的时候就设置给别人。这样启动时的性能就弱了一点，但是响应的性能就高了
...
序列化即钝化，内存中的对象是活的，我们把这个对象序列化到磁盘上去，把他钝化到磁盘上去。反序列化就是活化，就是将磁盘上的对象重新激活，重新加载到内存中
